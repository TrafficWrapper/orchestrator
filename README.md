...
`limits` accepts only `traffic_quota_bytes`, `rate_limit`, `expires_at`
(RFC3339) and `note`; any other field is refused.
...
| `ORCH_EGRESS_PROBE_URL` | Optional worker egress probe URL, used when a worker's ack comes from a private or loopback address (co-located worker). The worker's declared egress is otherwise checked against the ack's source address; a public mismatch raises an alert. | Optional | empty | Usually `http://127.0.0.1:9090/self-describe` in local dev. |
| `ORCH_EGRESS_ECHO_URLS` | Echo endpoints apps query through the tunnel to check their egress; sent in the client bundle as `egress_probe_urls`. | Optional | empty | Comma-separated `https://` URLs. |
| `ORCH_PUBLIC_TLS_SPKI_SHA256` | SPKI pins (current and backup) of the public certificate when TLS terminates at a proxy; sent in bootstrap as `orch_tls_spki_sha256`. With `ORCH_TLS=1` the orchestrator's own certificate is pinned. | Optional | empty | Comma-separated SHA-256, hex or base64. |
