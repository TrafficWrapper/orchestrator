...
| `ORCH_APK_MAX_BYTES` | Наибольший APK, принимаемый к публикации. | Опц. | `209715200` (200 MiB) | Байты, меньше лимита загрузки 256 MiB. |
...
| `ORCH_EGRESS_PROBE_URL` | Необязательный URL egress-пробы воркера; используется, когда ack воркера приходит с приватного или loopback-адреса (co-located воркер). Иначе объявленный egress воркера сверяется с адресом источника ack; расхождение публичных адресов даёт алерт. | Опц. | empty | Обычно `http://127.0.0.1:9090/self-describe` в local dev. |
| `ORCH_EGRESS_ECHO_URLS` | Echo-эндпоинты, которые app опрашивает через туннель для проверки своего egress; уходят в client bundle как `egress_probe_urls`. | Опц. | empty | `https://` URL через запятую. |
| `ORCH_PUBLIC_TLS_SPKI_SHA256` | SPKI-пины (текущий и резервный) публичного сертификата, когда TLS терминирует прокси; уходят в bootstrap как `orch_tls_spki_sha256`. При `ORCH_TLS=1` пинится собственный сертификат оркестратора. | Опц. | empty | SHA-256 через запятую, hex или base64. |
...
Затем задайте `ORCH_CLIENT_IP_HEADER` равным заголовку, который перезаписывает
ваш proxy (`x-real-ip` или `x-forwarded-for`). В режиме по умолчанию `auto`
proxy, выставляющий только один из них, позволяет клиенту подделать второй и
подменить свой IP для rate limit логина, аудита и подтверждений входа в Telegram.
