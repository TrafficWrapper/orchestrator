...
Noise-ответы enroll, pull, nudge, ack и telemetry (включая отказы) несут
`server_time` — часы оркестратора в Unix-миллисекундах. Ответ
`/w/v1/apk/chunk` объявляет это поле, но пока его не заполняет, поэтому
воркер не должен на него там полагаться. Воркер может держать смещение
